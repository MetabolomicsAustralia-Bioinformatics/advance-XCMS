...
def doStuff(roi):
    ...
    for i,a in enumerate(state['points']):
        rx = a.x()
        ry = a.y()
        oldPoints.append((x[i],y[i]))
        minx = np.argmin(np.abs(x - rx))
        miny = np.argmin(np.abs(y - ry))
    ...
    print newPoints
    print
    r2b.sigRegionChangeFinished.disconnect(doStuff)
    roi.setPoints(newPoints, closed = False)
    r2b.sigRegionChangeFinished.connect(doStuff)


x = np.asarray([1,2,2,3,4,4,5])
y = np.asarray([4,2,4,7,4,8,6])

vb = pg.ViewBox()
r2b = pg.PolyLineROI([[1,5], [2,3],[3,4], [4,2], [5,6]], closed=False, movable = False, removable = False)
r2b.sigRegionChangeFinished.connect(doStuff)
vb.addItem(r2b)

plt = pg.PlotItem(viewBox = vb)
plt.plot(x,y, pen = None, symbol = 'o')
w.addItem(plt, row = 0, col = 1)
