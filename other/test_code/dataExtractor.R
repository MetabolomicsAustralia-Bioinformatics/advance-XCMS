...
filepath <- '/media/sf_VM_share/singapore_batch_4_all/data'


# MS files should be grouped in folders below this directory.
datapath <- '/media/sf_VM_share/singapore_batch_4_all/data'
dir(datapath, recursive=TRUE)


# Load a "typical" MS file which can be considered a reference.
ref <- "/media/sf_VM_share/singapore_batch_4_all/data/PBQC/pbQC70.mzXML"

# Load all the MS files
rawfiles <- dir(datapath, full.names=TRUE,pattern="\\.mzXML*", recursive=TRUE)[1:3]
rawfiles


of1 <- paste('smallTestData.dat', sep = '')
...
##############################################################################
print ('rt correction')
xset3 <- fillPeaks(xs_align, method="chrom", BPPARAM = MulticoreParam(workers = 6))
