...
print (args)
datapath <- '/media/sf_VM_share/singapore_batch5_pbqd/pbqd' # as.character(args[1])
optimisationFileName <- 'optTest.dat'#as.character(args[2])
outputFileName <- 'output.dat' #as.character(args[3])
...
allowedMissing <- as.integer(length(rawfiles)/10)
...
#xset <- xcmsSet(rawfiles[1])
#iters <- 20
#for (i in 2:length(rawfiles)) {
#    print ( paste ( 'Generating xcmsSet for file', i ) )
#    xsetTemp <- xcmsSet(rawfiles[i])
#    xset <- c(xset, xsetTemp)
#    print ( paste ( 'NumPeaks in xSet is:', length(xset@peaks)))
#    for ( j in 1:iters) {
#        print ( paste ( 'RT alignment iteration:', j) )
#        xset <- group(
...
ref <- '/media/sf_VM_share/singapore_batch5_pbqd/pbqd/pbQC68.mzXML'
...
for (i in 1:numIter) {
	...
	max_pks2 <- max_pks * i
        print(paste('number of allowed missing peaks', allowedMissing))
	#xset <- retcor(xset, missing = allowedMissing)
	...
	xset <- group(xset, method= "density", bw= bw, minfrac= 0.5,
        ...
        xset <- retcor(xset, method= "obiwarp", center= align_ref,
